...
from zuperscore.api.views.subjects import (
    ...
    MoleculeSerializer,
    MoleculeTopicSubtopicSerializer,
    MotherSessionMoleculeSerializer,
    StudentSessionPlanSerializer,
    SubTopicSerializer,
    SubjectSerializer,
)
...
class StudentAssignmentQuestionSerializer(serializers.ModelSerializer):
    ...
    student_question_options = StudentQuestionOptionSerializer(
...
class StudentHomeAssignmentSerializer(serializers.ModelSerializer):
    subtopic = SubTopicPracticeSheetSerializer()
    student_assignment_questions = StudentAssignmentQuestionSerializer(
        many=True, read_only=True
    )
...
class AppointmentViewSet(BaseViewset, BasePaginator):
    ...
    def get_student_teachers(self, request, pk):
        try:
            ...
            if subject == "math":
                ...
                tutors = english_reading_tutors.union(
                    english_writing_tutors, math_tutors
                )

            if cpea:
                cpea_enabled_tutors = User.objects.filter(is_cpea_eligible=True)
                tutors = cpea_enabled_tutors.exclude(
                    id__in=tutors.values_list("id", flat=True)
                )
            ...
            for tutor in tutors:
                tutor_data_response = self.get_dayScheduler_resource_by_id(
                    request, tutor.day_schedule_user_id, slug
                )
                if tutor_data_response.status_code == 200:
                    ...
                    if cpea:
                        tutor_data = [
                            event for event in tutor_data if "CPEA" in event["name"]
                        ]
                        ...
                        anonymized_tutor = UserMinimumSerializer(
                            [tutor], many=True
                        ).data
    ...
    def admin_all_tutor_list(self, mega_domain, request, id, slug):
        ...
        all_student_tutors = (
            list(english_reading_tutors)
            + list(english_writing_tutors)
            + list(math_tutors)
        )

        subject_tutors = User.objects.filter(
            tutor_type=mega_domain.lower(),
            role="tutor",
            day_schedule_user_id__isnull=False,
        )
        ...
        if assigned_tutor:
            subject_tutors = [assigned_tutor] + [
                tutor for tutor in subject_tutors if tutor != assigned_tutor
            ]
    ...
    def get_dayScheduler_resource_by_id(self, request, tutor_id, slug):
        if not tutor_id:
            return Response(
                {"error": "Tutor DayScheduler ID is not provided or is invalid"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        params = {"user_id": tutor_id, "apiKey": "AOthHfwumTELV7qUslLHRNxeOpObRhvp"}
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if slug:
                    filtered_events = [
                        event for event in users_list if slug in event["slug"]
                    ]
                ...
                return Response(
                    {"error": response.status_code}, status=status.HTTP_400_BAD_REQUEST
                )
    ...
    def get_available_appointments(self, request, pk):
        get_available_appointments_url = (
            f"https://api.dayschedule.com/v1/availability/{pk}/"
        )
        ...
        try:
            response = requests.get(
                get_available_appointments_url, headers=headers, params=params
            )
            ...
            if response.status_code == 200:
                ...
                return Response(
                    {"error": response.status_code}, status=response.status_code
                )
    ...
    def slot_booking(self, request, type=None):
        try:
            ...
            if appointment_type not in ["coreprep", "cpea", "group_class"]:
                return Response(
                    {"error": "Invalid appointment type provided."},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            if not ds_host_id:
                return Response(
                    {"error": "Day-Scheduler id not found"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            try:
                ...
                return Response(
                    {"error": "Host user not found"}, status=status.HTTP_400_BAD_REQUEST
                )
            ...
            if request.user.role == "admin":
                ...
                estimated_end_time = new_start_datetime + timedelta(
                    minutes=MAX_DURATION_MINUTES
                )

                # Retrieve all appointments for the student on the same day.
                appointment_date = new_start_datetime.date()
                same_day_appointments = Appointments.objects.filter(
                    student=student, start_at__date=appointment_date
                )
                ...
                if appointment_type == "group_class":
                    existing_group_classes = Appointments.objects.filter(
                        student=student,
                        type="group_class",
                        start_at__date=appointment_date,
                        mega_domain=mega_domain,
                    )
                    if existing_group_classes.exists():
                        return Response(
                            {
                                "error": "You can only book one group class of the same mega domain at a time."
                            },
                            status=status.HTTP_400_BAD_REQUEST,
                        )
            ...
            payload = {
                ...
                "host": {"user_id": ds_host_id},
                "start_at": start_at,
                "location": {"name": "zoom", "type": "zoom"},
                "attendees": [{}],
                "invitees": [
                    {
                        "invitee_id": invitee_id,
            ...
            response = requests.post(
                slot_booking_url, headers=headers, params=params, json=payload
            )
            if response.status_code == 200:
                ...
                return Response(
                    {"error": response.status_code, "message": error_message},
    ...
    def get_coming_classes(self, request, invitee_id):
        ...
        try:
            bookings = (
                Appointments.objects.filter(invitee_id=invitee_id)
            ...
            for appointment_data, day_scheduler_response in zip(
                data, day_scheduler_responses
            ):
                zoom_link = (
                    day_scheduler_response.get("users_list", {})
                    .get("location", {})
                    .get("join_url")
                )
                subject = day_scheduler_response.get("users_list", {}).get(
                    "subject", {}
                )
                ...
                appointment = (
                    Appointments.objects.filter(id=appointment_id)
                    .exclude(status__in=["CANCELLED", "RESCHEDULED"])
                    .get()
                )
                if zoom_link:
                ...
                related_subtopics = MoleculeTopicSubtopic.objects.filter(
                    molecule_id__in=taught_molecules
                ...
                class_assignment_completed = (
                    related_class_assignments.exists()
                    and related_class_assignments.filter(is_completed=False).count()
                    == 0
                )
                ...
                are_all_home_assignments_completed = (
                    len(completed_home_assignments) == home_assignment_count
                )
                ...
                tutor_feedback_exists = FeedBack.objects.filter(
                    appointment_id=appointment_id,
                    tutor_id=appointment.host_id,
                    commenter="tutor",
                ).exists()

                appointment_data["class_assignment_completed"] = (
                    class_assignment_completed
                )
                appointment_data["home_assignment_completed"] = (
                    are_all_home_assignments_completed
                )
    ...
    def get_dayScheduler_booking(self, request, booking_id):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return Response(
                    {
                        "error": "Failed to retrieve booking info, status code: {}".format(
            ...
            return Response(
                {"error": "An error occurred while retrieving booking information."},
                status=500,
            )
    ...
    def get_all_users(self, request):
        ...
        try:
            response = requests.get(
                get_appointments_url, headers=headers, params=params
            )
    ...
    def get_appointments(self, request, tutor_id):
        ...
        params = {"id": tutor_id, "apiKey": "AOthHfwumTELV7qUslLHRNxeOpObRhvp"}
...
class StudentRelatedTeacherViewSet(BaseViewset, BasePaginator):
    ...
    def students_related_teacher(self, request):
        ...
        try:
            ...
            users = (
                users.english_reading_tutors.all().distinct()
            ...
            if name:
                ...
                users = users.filter(
                    ...
                    | Q(
                        ...
                        last_name__icontains=name.split()[-1],
                    )
                )
            ...
            serialized_students = UserMinimumSerializer(
                paginated_students, many=True
            ).data

            results = []
            for student_data in serialized_students:
                student_sessions = StudentSessionPlan.objects.filter(
                    student_id=student_data["id"]
                )
                serialized_student_sessions = StudentSessionPlanSerializer(
                    student_sessions, many=True
                ).data
                student_availability = (
                    StudentAvailability.objects.filter(student_id=student_data["id"])
                    .exclude(target_test_date_1__isnull=True)
                    .first()
                )
                serialized_availability = (
                    StudentAvailabilitySerializer(student_availability).data
...
class AppointmentAgendaViewSet(BaseViewset, BasePaginator):
        ...
        try:
            ...
            if appointment_id:
                query_set_data = AppointmentMolecule.objects.filter(
                    appointment_id=appointment_id, is_completed=True
                ...
                sorted_molecules = sorted(
                    molecules_list,
                    key=lambda x: (
                        int(re.search(r"\d+", x.title).group())
                        if re.search(r"\d+", x.title)
                        else float("inf")
                    ),
                )
                ...
                student_session_plans = StudentSessionPlan.objects.filter(
                ...
                for session_plan in student_session_plans:
                    ...
                    mega_domain_name = (
                        session_plan.mega_domain.name.lower()
                        if session_plan.mega_domain
                        else ""
                    )
                    ...
                    if subject_name == "english":
                        if not (
                            (
                                "reading" in mega_domain_name
                                and tutor_subject_type == "english_reading"
                            )
                            or (
                                "writing" in mega_domain_name
                                and tutor_subject_type == "english_writing"
                            )
                        ):
                            continue
                    ...
                    total_number_of_molecule = MotherSessionMolecule.objects.filter(
                    ...
                    if subject_name == "english":
                        ...
                        if category == "R":
                            ...
                            per_day_molecule = (
                                total_number_of_molecule / total_number_of_classes
                            )
                        elif category == "S":
                    ...
                    existing_molecule_ids = AppointmentMolecule.objects.filter(
                        appointment__student=student_id, is_completed=True
                    ).values_list("molecule_id", flat=True)

                    query_set_data = MotherSessionMolecule.objects.filter(
                        session_plan=session_plan.session_plan.id
                    ).exclude(molecule_id__in=existing_molecule_ids)
                    ...
                    sorted_molecules = sorted(
                        molecules_list,
                        key=lambda x: (
                            int(re.search(r"\d+", x.molecule.title).group())
                            if re.search(r"\d+", x.molecule.title)
                            else float("inf")
                        ),
                    )
                    ...
                    serializer = MotherSessionMoleculeSerializer(
                        final_molecules, many=True
                    )
    ...
    def createAppointmentMolecules(self, request):
        try:
            ...
            for molecule_id in molecules_data:
                if not AppointmentMolecule.objects.filter(
                    appointment=appointment, molecule_id=molecule_id
                ).exists():
            ...
            return Response(
                {"message": "Molecules Saved..!!"}, status=status.HTTP_200_OK
            )
        except Exception as e:
            print("Error Occurred:", e)
            return Response(
    ...
    def update_appointment_molecules_and_feedback(self, request):
        try:
            ...
            if user_type == "tutor":
                ...
                appointment_molecules = AppointmentMolecule.objects.filter(
                    appointment=appointment
                )
            ...
            for feedback_question in feedback_questions:
                ...
                FeedbackQuestionRating.objects.create(
                    feedback=feedback, question=question, rating=rating
                )
            ...
            return Response(
                {"message": "Feedback and molecules updated successfully."},
    ...
    def get_other_covered_agenda(self, request, student_id):
        try:
            ...
            for session_plan in student_session_plans:
                subject_name = (
                    session_plan.subject.name.lower() if session_plan.subject else ""
                )
                mega_domain_name = (
                    session_plan.mega_domain.name.lower()
                    if session_plan.mega_domain
    ...
    def get_last_classes_agenda(self, request):
        ...
        if last_completed_appointment:
            last_classes_agenda = AppointmentMolecule.objects.filter(
                appointment=last_completed_appointment, is_completed=True
            )
            molecule = last_classes_agenda.values_list("molecule", flat=True)

            molecule_topic_subtopic = MoleculeTopicSubtopic.objects.filter(
                molecule_id__in=molecule
            )

            home_assignment_status = StudentAssignment.objects.filter(
                subtopic__in=molecule_topic_subtopic.values_list("subtopic", flat=True),
                student_id=student_id,
                type="HOME",
                is_completed=True,
            ).exists()

            appointment_serializer = NewAppointmentSerializer(
                last_completed_appointment
            )
            last_class_agenda_serializers = AppointmentMoleculeSerializers(
                last_classes_agenda, many=True
            )
...
class TeacherAppointmentViewSet(BaseViewset, BasePaginator):
    ...
    def get_teachers_appointments(self, request):
        ...
        if appointment_status == "completed":
            appointments = (
                appointments.filter(
                    Q(is_completed=True)
                    | (
                        Q(start_at__lt=now - timedelta(hours=1, minutes=30))
            ...
            appointments = appointments.filter(is_completed=False).exclude(
        ...
        for app in serializer:
            if app["type"] == "group_class":
                if app["resource_id"] in appointment_resource_grouping_map:
                    appointment_resource_grouping_map[app["resource_id"]].append(
                        app["student"]
                    )

                else:
                    appointment_resource_grouping_map[app["resource_id"]] = [
                        app["student"]
                    ]
        ...
        for appointment in appointments:
            ...
            has_molecules = AppointmentMolecule.objects.filter(
                appointment__id=appointment.id, **appointment_filter
            ).exists()
            ...
            if total_assignments_count == 0:
                ...
                completed_assignments_count = related_assignments.filter(
                    is_completed=True
                ).count()
                class_assignment_completed = (
                    total_assignments_count == completed_assignments_count
                )
            ...
            if appointment_data["type"] == "group_class":
                appointment_data["student_count"] = len(
                    appointment_resource_grouping_map.get(
                        appointment_data["resource_id"], []
                    )
                )

            appointment_obj = AppointmentReport.objects.filter(
                appointment=appointment.id
            ).first()
            appointment_data["is_tutor_joined"] = (
                appointment_obj.is_tutor_joined if appointment_obj else False
            )
...
class AppointmentAssignemntViewset(BaseViewset, BasePaginator):
    ...
    def get_student_assignment_question(self, request, student_assignment_id):
        try:
            ...
            student_assignment_serialized = StudentAssignmentSerializer(
                student_assignment
            ).data

            questions = StudentAssignmentQuestion.objects.filter(
                student_assignment=student_assignment
            ).prefetch_related("student_question_options")
            questions_serialized = StudentAssignmentQuestionSerializer(
                questions, many=True
            ).data
            ...
            for option in options:
                ...
                try:
                    question_option = StudentQuestionOption.objects.get(
                        id=option_id, student_assignment_question__id=question.get("id")
                    )
        ...
        for student_assignment_id in student_assignment_ids:
            try:
                ...
                if is_submit_clicked:
                    ...
                    messages.append(
                        f"Assignment {student_assignment_id} marked as completed successfully."
                    )
                else:
                    pass
            except StudentAssignment.DoesNotExist:
    ...
    def correct_answer_for_questions(self, request, student_id):
        ...
        for question_id in question_ids:
            correct_options_qs = QuestionOption.objects.filter(
                questions_id=question_id, is_correct=True
            ).prefetch_related("questions")

            correct_options_serializer = QuestionOptionSerializer(
                correct_options_qs, many=True
            )

            # Filter student_answers for the current question
            filtered_student_answers = [
                ans
                for ans in student_answers_qs
            ...
            validation = [
                {
                    ...
                    "selected_option_id": (
                        answer.question_option.id if answer.question_option else None
                    ),
...
class CompletedClassViewSet(BaseViewset, BasePaginator):
    ...
    def get_completed_classes(self, request, student_id):
        ...
        appointments = (
            Appointments.objects.filter(is_completed=True, student_id=student_id)
            .exclude(status__in=["CANCELLED", "RESCHEDULED"])
            .order_by("created_at")
        )
        ...
        for appointment in appointments:
            ...
            if appointment.type == "cpea":
                ...
                if student_feedback_exists and tutor_feedback_exists:
                    ...
                    appointment.home_assignment_present = (
                        has_home_assignments  # _Transient_field
                    )
        ...
        if sort_order == "desc":
            appointments = Appointments.objects.filter(
                is_completed=True, student_id=student_id
            ).order_by("-created_at")
        else:
            appointments = Appointments.objects.filter(
                is_completed=True, student_id=student_id
        ...
        serializer = AppointmentSerializer(
            paginated_appointment, many=True, context={"request": request}
        )
        return Response(serializer.data)
...
class LastClassViewSet(BaseViewset, BasePaginator):
    ...
    def get_last_classes(self, request, student_id):
        last_class = (
            Appointments.objects.filter(student_id=student_id, is_completed=True)
            .order_by("-start_at")
            .first()
        )
...
class StudentAvailabilityViewSet(BaseViewset):
    ...
    def create(self, request, pk):
        try:
            ...
            if (
                hasattr(student_user, "goal_post")
                and student_user.goal_post is not None
            ):
    ...
    def update_days(self, request, student_id):
        try:
            ...
            if "type" in entry and entry["type"] == "black_out":
                student_availability = student_availability.filter(
                    type="black_out"
                ).first()
            ...
            if (
                "total_days" in entry
                and entry["total_days"] is not None
                and student_availability is not None
            ):
    ...
    def get_by_id(self, request, pk):
        try:
            ...
            for availability in student_availability:
                ...
                recent_target_test_date = max(
                    (i for i in recent_target_test_dates if i is not None)
                )
                ...
                if (
                    recent_update_tuple != ()
                    and recent_update_tuple[0] < recent_target_test_date
                ):
                    print("run inside if")
                    recent_update_tuple = (
                        recent_target_test_date,
                        recent_core_prep_date,
                    )
    ...
    def get(self, request):
        try:
            ...
            serializer = StudentAvailabilitySerializer(
                student_availabilities, many=True
            )
            return Response(serializer.data, status=status.HTTP_200_OK)
...
class CalculateStudentAvailabilityViewSet(BaseViewset):
    ...
    def calculate_availability(self, request, student_id):
        try:
            student_availabilities = StudentAvailability.objects.filter(
                student=student_id
            )
            ...
            test_tracker = 0
            student_check = student_availabilities.filter(
                target_test_date_1__isnull=False
            )
            if not student_check.exists() and test_tracker == 0:
                test_tracker = 1
            ...
            disposable_time = (
                math.ceil(number_of_weeks)
                - math.ceil(total_blackout_weeks)
                + 0.5 * math.ceil(total_accelerator_weeks)
            )
            student_session_plans = StudentSessionPlan.objects.filter(
                student=student_id
            )
            ...
            temp_dict["total_accelerator_weeks"] = abs(
                math.ceil(total_accelerator_weeks)
            )
            ...
            for session_plan in student_session_plans:
                ...
                if subject_name:
                    number_of_molecule = MotherSessionMolecule.objects.filter(
                        session_plan=session_plan.session_plan.id
                    ).count()
            ...
            temp_dict["english_number_of_molecules"] = (
                temp_dict.get("english_number_of_molecules", 0)
                + english_number_of_molecules
            )
            temp_dict["math_number_of_molecules"] = (
                temp_dict.get("math_number_of_molecules", 0) + math_number_of_molecules
            )
...
class AssignCategoryViewSet(BaseViewset):
    ...
    def assign_category(self, request, student_id):
        ...
        try:
            ...
            return Response(
                {"success": "Test results processed successfully"},
                status=status.HTTP_200_OK,
            )
    ...
    def get_user_teams(self, request, user_id):
        try:
            ...
            if user.role == "user" and user.english_category and user.math_category:
                print("enter")
                english_reading_tutors = User.objects.filter(
                    role="tutor",
                    tutor_type="english_reading",
                    english_category=user.english_category,
                )
                english_writing_tutors = User.objects.filter(
                    role="tutor",
                    tutor_type="english_writing",
                    english_category=user.english_category,
                )
                math_tutors = User.objects.filter(
                    role="tutor", tutor_type="math", math_category=user.math_category
                )
...
class TutorDashBoradViewSet(BaseViewset):
    ...
    def tutor_dashboard(self, request):
        ...
        upcoming_appointments = (
            Appointments.objects.filter(
                host=logged_in_user_id, start_at__gt=current_time, is_completed=False
            )
            .exclude(status__in=["CANCELLED", "RESCHEDULED"])
            .order_by("start_at")
        )

        total_feedback = FeedBack.objects.filter(tutor=logged_in_user_id)
...
class TeacherAppointmentFeedbackViewSet(BaseViewset):
    ...
    def list(self, request, appointment_id):
        try:
            ...
            response_data = {"appointment_molecule": [], "feedback": []}

            appointment_molecules = AppointmentMolecule.objects.filter(
                appointment_id=appointment_id
            ).order_by("created_at")
            response_data["appointment_molecule"] = AppointmentMoleculeSerializers(
                appointment_molecules, many=True
            ).data

            all_feedback_questions = FeedbackQuestions.objects.filter(
                type=feedback_type
            )
            ...
            if student_id:
                feedback_filter &= Q(student_id=student_id) & (
                    Q(commenter="tutor") | Q(commenter="user")
                )

            feedback_entries = FeedBack.objects.filter(feedback_filter).order_by(
                "-created_at"
            )
            ...
            if feedback_entries:
                for fb in feedback_entries:
                    ...
                    comment_key = (
                        "tutor_comment"
                        if fb.commenter == "tutor"
                        else "student_comment"
                    )
                    feedback_serializer[comment_key] = feedback_serializer.pop(
                        "comment"
                    )

                    for question in all_feedback_questions:
                        rating = FeedbackQuestionRating.objects.filter(
                            feedback_id=fb.id, question=question
                        ).first()
                        rating_data = (
                            FeedbackQuestionRatingSerializer(rating).data
                            if rating
                            else None
                        )
                    ...
                    feedback_serializer["feedback_question_ratings"] = (
                        feedback_question_ratings_data
                    )
            ...
            return Response(
                {"success": True, "status": "success", "results": [response_data]},
                status=status.HTTP_200_OK,
            )
...
class StudentDashBoardViewSet(BaseViewset):
    ...
    def get_test_tracker(self, student_availabilities):
        for i in range(1, 5):
            student_check = student_availabilities.filter(
                **{f"target_test_date_{i}__isnull": False}
            )
    ...
    def get_session_completion(
        self, student_session_plans, student_id, tutors_assigned
    ):
        session_completed = {
    ...
    def calculate_total_classes(self, student_session_plans):
        ...
        for session_plan in student_session_plans:
            ...
            category = session_plan.category.upper()
            number_of_molecule = MotherSessionMolecule.objects.filter(
                session_plan=session_plan.session_plan_id
            ).count()
            if subject_name == "english":
                if category == "R":
        ...
        total_classes = math.ceil(sum(english_number_of_molecules_list)) + math.ceil(
            sum(math_number_of_molecules_list)
        )
    ...
    def calculate_student_progress(self, request, student_id):
        try:
            ...
            course_end_date, exam_date = self.get_course_and_exam_dates(
                student_availabilities, test_tracker
            )
            ...
            total_blackout_days, total_accelerator_days = self.calculate_total_days(
            ...
            proportion_of_course_elapsed = (
                elapsed_duration_days / total_course_duration_days
                if total_course_duration_days > 0
                else 0
            )
            ...
            total_attended_classes = Appointments.objects.filter(
                student_id=student_id, is_completed=True, is_active=True
            ).count()
            ...
            results = {
                ...
                "english_writing_session_completed": session_completed[
                    "english_writing"
                ],
                "english_reading_session_completed": session_completed[
                    "english_reading"
                ],
...
class CpeaBaseViewSet(BaseViewset):
    ...
    def get_cpea_assignment(self, request, mega_domain):
        ...
        assignments = Assignment.objects.filter(
            type__startswith=mega_domain_prefix, is_active=True
        )
...
class GroupClassesBaseViewSet(BaseViewset):
    ...
    def get_dayScheduler_group_events(self, request, student_id):
        try:
            ...
            student_group_events = StudentGroupEvents.objects.filter(
                student_id=student_id, subject=subject, is_sso_verified=True
            )
            ...
            for student_id in student_ids:
                ...
                for class_event in class_events:
                    ...
                    if not (tutor_id and tutor_name and class_name and event_id):
                        return Response(
                            {"error": "Incomplete class event information provided."},
                            status=400,
                        )
...
class StudentGroupEventBaseViewSet(BaseViewset):
    ...
    def get_student_group_events(self, request):
        ...
        for group_event in paginated_students:
            events = StudentGroupEvents.objects.filter(
                group_id=group_event["group_id"], sso=logged_in_user_sso
            ).values(
                "event_id",
                "class_name",
                "tutor_name",
...
class AllotGroupClassBaseViewSet(BaseViewset):
    ...
    def allot_group_classes(self, request):
        try:
            ...
            for student_id in student_ids:
                StudentGroupEvents.objects.filter(
                    student_id=student_id,
                    is_sso_verified=False,
                    subject=subject,
                    group_id=group_id,
                ).update(is_sso_verified=True)
    ...
    def check_student_group_assignment(self, request, student_id):
        try:
            ...
            for subject in subjects:
                group_class_assigned = StudentGroupEvents.objects.filter(
                    student_id=student_id, subject=subject, is_sso_verified=True
                ).exists()
        ...
        try:
            ...
            if subject == "English":
                user_queryset = user_queryset.filter(
                    english_category__name__in=["R", "S", "T"]
                )
            elif subject == "Math":
                user_queryset = user_queryset.filter(
                    math_category__name__in=["R", "S", "T"]
                )
            ...
            for user in user_queryset:
                user_group_event_status = (
                    student_events.filter(student=user).first().is_sso_verified
                )
                categories_matched = []
                if (
                    subject == "English_Reading"
                    or subject == "English_Writing"
                    or subject == "English"
                    and hasattr(user, "english_category")
                    and user.english_category.name in ["R", "S", "T"]
                ):
                    ...
                    and user.math_category.name in ["R", "S", "T"]
                ):
...
class ReportClassesViewSet(BaseViewset):
    ...
    def create(self, request, appointment_id):
        try:
            ...
            return Response(
                {"error": "Invalid Appointment"}, status=status.HTTP_404_NOT_FOUND
            )
        ...
        try:
            ...
            appointment_report.is_student_joined = (
                is_student_joined or appointment_report.is_student_joined
            )
            appointment_report.is_tutor_joined = (
                is_tutor_joined or appointment_report.is_tutor_joined
            )
            appointment_report.save()
...
class CpeaOverRideViewSet(BaseViewset):  # added after merger
    permission_classes = ((IsPlatformAdmin | IsManager),)
    ...
    def cpea_override(self, request, student_id, mega_domains):
        ...
        student_session_plans = StudentSessionPlan.objects.filter(
            student_id=student_id, mega_domain__in=mega_domain_objects
        )
...
class UnattendedClassesViewSet(BaseViewset):
    permission_classes = ((IsPlatformAdmin | IsManager),)
    ...
    def list(self, request):
        ...
        if (
            request.user.role.lower() == "sso_manager"
            or "admin"
            or "manager"
            or "prep_manager"
        ):
            ...
            if appointment_id and action in ["cancel", "reschedule"]:
                try:
                    appointment = Appointments.objects.get(id=appointment_id)
                    appointment.status = (
                        "CANCELLED" if action == "cancel" else "RESCHEDULED"
                    )
                    ...
                    return Response(
                        {
                            "sucess": True,
                            "status": "sucess",
                            "message": "class canceled or rescheduled sucessfully..",
                        },
                        status=status.HTTP_201_CREATED,
                    )
                except Appointments.DoesNotExist:
            ...
            return Response(
                {"error": "You do not have permission to cancel the class"},
                status=status.HTTP_403_FORBIDDEN,
            )


class UnattendedCounterClassesViewSet(BaseViewset):
    permission_classes = ((IsStudent | IsPlatformAdmin),)
    ...
    def counters_of_all_classes(self, request):
        ...
        if student_id:
            ...
            student_no_show_classes = (
                AppointmentReport.objects.filter(
                    is_student_joined=False, appointment__student_id=student_id
                )
                .exclude(
                    Q(appointment__status="CANCELLED")
                    | Q(appointment__status="RESCHEDULED")
                )
                .count()
            )
            ...
            cancelled_class = (
                ...
                .exclude(appointment_reports__is_student_joined=False)
                .count()
            )
            ...
            reschedule_class = (
                ...
                .exclude(appointment_reports__is_student_joined=True)
                .count()
            )
